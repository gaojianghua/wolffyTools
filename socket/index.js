class WS {
	...
	initWS() {
		...
		const url = (this.options.https ? 'wss://' : 'ws://') + this.options.config.baseURL + (this.options.data.userId ? '/' + this.options.data.userId : '')
	...
	onReconnect() {
		clearTimeout(this.reconnectTimer)
		if (this.reconnectTime < this.options.config.maxReconnectMaxTime) {
